...
if (isset($_GET['codigo'])||isset($_POST['ok'])){
    ...
    if(empty($conn2->getRow("SELECT id FROM usuarios WHERE clave_activacion = '$idval'"))){
        mensajeError("Código de activación incorrecto.",null);
        goto error;
    }
    #chequeo si el codigo ya ha sido activado
    if(!empty($conn2->getRow("SELECT id FROM usuarios WHERE clave_activacion = '$idval' AND activo=1"))){
