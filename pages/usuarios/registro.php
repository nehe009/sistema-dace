<?php
    ...
    if(isset($_POST['ok'])){ #Comprobamos si se a pulsado el boton OK
        ...
        if(isset($_SESSION['security_code_login']) && !empty($_SESSION['security_code_login']) && $_POST['inputCaptcha'] != $_SESSION['security_code_login']){
            mensajeError("Captcha incorrecto.");
        } else {       
            extract($_POST);
            //print_r($_POST);
            ...
            $ar = $conn->getRow($sql);
            if(empty($ar)){
                mensajeError("Usted no es estudiante de esta institución.");
            } else {
                ...
                $ar = $conn->getRow($sql);
                if(!empty($ar)){
                    mensajeError("Esta cedula de identidad ya se encuentra registrada.");
                    } else {
