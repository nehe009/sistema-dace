...
if(isset($_POST["obtenerPais"]) && is_numeric($_POST["obtenerPais"])){    
    #consulta de paises.
    $datos = $conn2->getAll("SELECT id_pais, pais FROM localidad_paises ORDER BY pais ASC");    
...
if(isset($_POST["obtenerEstado"]) && is_numeric($_POST["obtenerEstado"])){    
    #consulta por id
    $datos = $conn2->getAll("SELECT id_estado, estado FROM localidad_estados ORDER BY estado ASC");    
...
if(isset($_POST["obtenerCiudad"]) && is_numeric($_POST["obtenerCiudad"])){    
    #consulta por id
    $datos = $conn2->getAll("SELECT id_ciudad, ciudad FROM localidad_ciudades WHERE id_estado='$_POST[obtenerCiudad]'");  
...
if(isset($_POST["obtenerMunicipio"]) && is_numeric($_POST["obtenerMunicipio"])){    
    #consulta por id
    $datos = $conn2->getAll("SELECT id_municipio, municipio FROM localidad_municipios WHERE id_estado='$_POST[obtenerMunicipio]'");  
...
if(isset($_POST["obtenerParroquia"]) && is_numeric($_POST["obtenerParroquia"])){    
    #consulta por id
    $datos = $conn2->getAll("SELECT id_parroquia, parroquia FROM localidad_parroquias WHERE id_municipio='$_POST[obtenerParroquia]'");  
...
if(isset($_POST["useronline"]) && is_numeric($_POST["useronline"])){    
    ...
    $conn2->Execute("UPDATE usuarios SET online=1 WHERE ced_usu='$_POST[useronline]'");
}
#consulta ajax para usuario online
if(isset($_POST["getuseronline"]) && is_numeric($_POST["getuseronline"])){
$conexion=0;    
    #consulta de usuarios online
    $datos = $conn2->getAll("SELECT usuario, ced_usu, fecha_ultimo_acceso, corr_usu FROM usuarios WHERE online=1"); 
